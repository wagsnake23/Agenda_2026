...
const ConfirmDialog: React.FC<{
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 w-full max-w-sm z-10 animate-in zoom-in-95 duration-200">
                <p className="text-slate-700 font-medium text-sm text-center mb-6">{message}</p>
                <div className="flex gap-3">
                    <button
                        onClick={onCancel}
                        className="flex-1 h-10 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50 transition-all"
                    >
                        Cancelar
                    </button>
                    <button
                        onClick={onConfirm}
                        className="flex-1 h-10 rounded-xl bg-red-600 text-white font-bold text-sm hover:bg-red-700 transition-all"
                    >
                        Excluir
                    </button>
                </div>
